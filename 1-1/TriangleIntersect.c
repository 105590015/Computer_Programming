...
main()
{
    int n,i=0,j,k,l,p,ma=0,sum=1,num=0,b1,b2,s1=0,s2=0;
    int x[50]={0},y[50]={0};
    double xfocus[50]={0},yfocus[50]={0},t1,t2,xo,yo;
    scanf("%d",&n);
    while(i<(n*3))
    {
        scanf("%d %d",&x[i],&y[i]);
    ...
    for(l=0;l<ma;l++)
    {
        for(p=l+1;p<ma;p++)
        {
            if(xfocus[l]==xfocus[p]&&yfocus[l]==yfocus[p]&&xfocus[l]!=0&&yfocus[l]!=0)
            {
                xfocus[p]=0;
                yfocus[p]=0;
                s1++;
